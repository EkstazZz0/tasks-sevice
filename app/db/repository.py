...
async def get_tasks(session: AsyncSession, limit: int, offset: int, status: TaskStatus | None = None) -> list[Task]:
...
async def update_task(session: AsyncSession, db_task: Task, update_task_data: TaskUpdate) -> Task:
